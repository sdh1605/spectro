...
Verwendung:
    python spec_flux.py ARBEITSORDNER SPEKTRALTYP [OPTIONS]
...
Beispiele:
    ...
    # Response-Glättungsmethoden
    python spec_flux.py vega A0V --response-method polynomial --poly-order 5
    python spec_flux.py vega A0V --response-method spline --spline-knots 20
    python spec_flux.py vega A0V --response-method smooth  # alte Methode
...
WICHTIG: Die Response sollte eine GLATTE Kurve ohne Spektrallinien sein!

Drei Methoden zur Response-Glättung verfügbar:

1) polynomial (EMPFOHLEN, Standard):
   - Passt Polynom (Ordnung 3-6) an die Response-Daten an
   - Eliminiert automatisch ALLE Linien (H-Linien, telluric)
   - Sehr glatt, physikalisch sinnvoll
   - Keine manuelle Maskierung nötig
   
2) spline:
   - Passt Spline mit wenigen Knoten an
   - Flexibler als Polynom, kann lokale Variationen erfassen
   - Immer noch glatt genug um Linien zu eliminieren
   - Keine manuelle Maskierung nötig
   
3) smooth (alte Methode):
   - Maskiert tellurische Banden explizit
   - Interpoliert maskierte Bereiche
   - Starke Gauss-Glättung
   - H-Linien werden nur durch Glättung reduziert
...
def calculate_instrumental_response(wavelength_obs, flux_obs, wavelength_ref, flux_ref, 
                                   method='polynomial', poly_order=4, spline_knots=15):
    """
    ...
    WICHTIG: Die Response sollte eine GLATTE Kurve sein, ohne Spektrallinien!
    
    Drei Methoden zur Glättung verfügbar:
    
    1) 'polynomial' (EMPFOHLEN): Passt Polynom an die Response an
       - Sehr glatt, physikalisch sinnvoll
       - Eliminiert automatisch alle Linien (H-Linien, telluric)
       - Gut für breite Wellenlängenbereiche
       - Parameter: poly_order (Standard: 4)
    
    2) 'spline': Passt Spline mit wenigen Knoten an
       - Flexibler als Polynom
       - Kann lokale Variationen besser erfassen
       - Immer noch glatt genug um Linien zu eliminieren
       - Parameter: spline_knots (Standard: 15)
    
    3) 'smooth': Interpoliert tellurische Regionen + Gauss-Glättung
       - Alte Methode (backward compatibility)
       - Maskiert nur tellurische Banden explizit
       - H-Linien müssen durch starke Glättung entfernt werden
    ...
    method : str
        Methode zur Response-Glättung: 'polynomial', 'spline', oder 'smooth'
    poly_order : int
        Polynom-Ordnung (nur für method='polynomial')
    spline_knots : int
        Anzahl Knoten für Spline (nur für method='spline')
    
    Returns:
    --------
    response_raw : array
        Rohe instrumental response (ungeglättet, mit Linien)
    response_smooth : array
        Geglättete instrumental response (glatte Kurve ohne Linien)
    telluric_mask : array (bool) oder None
        Maske der tellurischen Bereiche (nur bei method='smooth')
    """
    ...
    # Berechne rohe Response
    response_raw = flux_obs / (flux_ref_interp + 1e-10)
    
    # Entferne NaN und Inf Werte für Fitting
    valid_mask = np.isfinite(response_raw) & (response_raw > 0)
    
    if np.sum(valid_mask) < 10:
        print("  ⚠ Warnung: Zu wenig gültige Datenpunkte für Response-Fitting!")
        return response_raw, response_raw, None
    
    # === METHODE 1: POLYNOMIAL FIT ===
    if method == 'polynomial':
        print(f"  → Methode: Polynom-Anpassung (Ordnung {poly_order})")
        
        # Normalisiere Wellenlängen für bessere numerische Stabilität
        wave_norm = (wavelength_obs - wavelength_obs.mean()) / wavelength_obs.std()
        
        # Fitte Polynom nur an gültige Punkte
        poly_coeffs = np.polyfit(wave_norm[valid_mask], response_raw[valid_mask], poly_order)
        response_smooth = np.polyval(poly_coeffs, wave_norm)
        
        # Statistik
        residuals = response_raw[valid_mask] - response_smooth[valid_mask]
        rms = np.sqrt(np.mean(residuals**2))
        print(f"     Polynom RMS: {rms:.2e} ({100*rms/np.mean(response_raw[valid_mask]):.1f}%)")
        
        telluric_mask = None
        
    # === METHODE 2: SPLINE FIT ===
    elif method == 'spline':
        print(f"  → Methode: Spline-Anpassung ({spline_knots} Knoten)")
        
        from scipy.interpolate import UnivariateSpline
        
        # Erstelle Spline mit wenigen Knoten für Glättung
        # s=None bedeutet: passe alle Punkte an, aber mit Glättung durch wenige Knoten
        try:
            # Berechne Knoten-Positionen (gleichmäßig verteilt)
            knot_indices = np.linspace(0, len(wavelength_obs[valid_mask])-1, 
                                      spline_knots, dtype=int)
            
            # UnivariateSpline mit Glättung
            # s-Parameter kontrolliert Glättung (None = automatisch)
            spline = UnivariateSpline(wavelength_obs[valid_mask], 
                                     response_raw[valid_mask], 
                                     k=3,  # kubischer Spline
                                     s=len(valid_mask)*np.var(response_raw[valid_mask])*0.1)
            
            response_smooth = spline(wavelength_obs)
            
            # Statistik
            residuals = response_raw[valid_mask] - response_smooth[valid_mask]
            rms = np.sqrt(np.mean(residuals**2))
            print(f"     Spline RMS: {rms:.2e} ({100*rms/np.mean(response_raw[valid_mask]):.1f}%)")
            
        except Exception as e:
            print(f"  ⚠ Warnung: Spline-Fitting fehlgeschlagen ({e}), falle zurück auf Polynom")
            # Fallback auf Polynom
            wave_norm = (wavelength_obs - wavelength_obs.mean()) / wavelength_obs.std()
            poly_coeffs = np.polyfit(wave_norm[valid_mask], response_raw[valid_mask], poly_order)
            response_smooth = np.polyval(poly_coeffs, wave_norm)
    ...
    # === METHODE 3: SMOOTH (alte Methode mit Maskierung) ===
    elif method == 'smooth':
        print(f"  → Methode: Interpolation + Gauss-Glättung (alte Methode)")
        ...
        # Interpoliere tellurische Bereiche im beobachteten Spektrum
        flux_obs_corrected = flux_obs.copy()
        
        if np.any(telluric_mask):
            valid_mask_interp = ~telluric_mask & np.isfinite(flux_obs)
            
            if np.sum(valid_mask_interp) > 10:
                print(f"     Interpoliere {np.sum(telluric_mask)} tellurische Pixel")
                
                interp_flux = interp1d(wavelength_obs[valid_mask_interp], 
                                      flux_obs[valid_mask_interp],
                                      kind='linear', bounds_error=False,
                                      fill_value='extrapolate')
                flux_obs_corrected[telluric_mask] = interp_flux(wavelength_obs[telluric_mask])
        
        # Berechne korrigierte Response
        response_corrected = flux_obs_corrected / (flux_ref_interp + 1e-10)
        
        # Starke Glättung um H-Linien zu entfernen
        response_smooth = gaussian_filter1d(response_corrected, sigma=10.0)
        print(f"     Glätte mit Gauss-Filter (sigma=10.0)")
        
    else:
        raise ValueError(f"Unbekannte Methode: {method}. Verwende 'polynomial', 'spline', oder 'smooth'")
...
class FluxCalibrator:
    ...
    def __init__(self, work_dir, spectral_type, use_pickles=True, show_plot=True,
                 response_method='polynomial', poly_order=4, spline_knots=15):
        """
        Initialisierung
        ...
        response_method : str
            Methode für Response-Glättung: 'polynomial', 'spline', oder 'smooth'
        poly_order : int
            Polynom-Ordnung (für method='polynomial')
        spline_knots : int
            Anzahl Spline-Knoten (für method='spline')
        """
    ...
    def save_results(self):
        ...
        primary_hdu.header['METHOD'] = ('PICKLES' if self.use_pickles else 'SYNTHETIC', 
                                        'Calibration method')
        if self.use_pickles:
            primary_hdu.header['RESPMET'] = (self.response_method, 'Response smoothing method')
            if self.response_method == 'polynomial':
                primary_hdu.header['POLYORD'] = (self.poly_order, 'Polynomial order')
            elif self.response_method == 'spline':
                primary_hdu.header['SPKNOTS'] = (self.spline_knots, 'Spline knots')
    ...
    def run(self):
        ...
        if self.use_pickles:
            print(f"Response-Glättung: {self.response_method}", end="")
            if self.response_method == 'polynomial':
                print(f" (Ordnung {self.poly_order})")
            elif self.response_method == 'spline':
                print(f" ({self.spline_knots} Knoten)")
            else:
                print()
...
Response-Glättungsmethoden (nur bei Pickles):
  polynomial: Passt Polynom an Response (Standard, sehr glatt)
  spline:     Passt Spline mit wenigen Knoten an (flexibler)
  smooth:     Interpoliert telluric + Gauss-Filter (alte Methode)
...
angewendet werden.
    ...
    parser.add_argument('--response-method', choices=['polynomial', 'spline', 'smooth'],
                       default='polynomial',
                       help='Methode zur Response-Glättung (Standard: polynomial)')
    parser.add_argument('--poly-order', type=int, default=4,
                       help='Polynom-Ordnung für --response-method polynomial (Standard: 4)')
    parser.add_argument('--spline-knots', type=int, default=15,
                       help='Anzahl Spline-Knoten für --response-method spline (Standard: 15)')
